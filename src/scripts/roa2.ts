...
const LOG: Logger = new Logger("roa2");
...
export function getRivalByName(name: string): Rival | undefined {
	...
	if (!rival) {
		LOG.error(`Invalid rival name: "${name}"`);
	}
	return rival;
}
...
export function getRivalIconPathByName(name: string): string {
	return getRivalIconPath(getRivalByName(name));
}
...
export function getStageByName(name: string): Stage | undefined {
	const stageName = name as StageName;
	const stage: Stage | undefined = STAGES.find((stage) => stage.name === stageName);
	if (!stage) {
		LOG.error(`Invalid stage name: "${name}"`);
	}
	return stage;
}
 ...
 */
export function getStageIconPathByName(name: string): string {
	return getStageIconPath(getStageByName(name as StageName));
}
